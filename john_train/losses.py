...
def controller_loss_terms(
    ...
    cfg: Any,
    w_final_pos: float = 1.0,
    w_final_angle: float = 1.0,
    w_action_penalty: float = 0.0,
    w_jackknife: float = 1.0,
    ...
    action_pen = torch.tensor(0.0, device=actions.device, dtype=actions.dtype)
    for n in range(actions.shape[0]):
        violation = torch.relu(actions[n].abs() - cfg.soft_steering_max_rad)
        action_pen = action_pen + violation.pow(2).sum()

    total_pen = (
        w_final_pos * (final_x_pen + final_y_pen)
        + w_final_angle * (final_theta0_pen + final_theta1_pen)
        + w_jackknife * jackknife_pen
        + w_boundary * boundary_pen
        + w_action_penalty * action_pen
    )
